...
axis_color <- ggplot2::theme(axis.line = element_line(color = axis_line_color))
axis_labs <- ggplot2::theme(axis.title = element_text(face = "bold", size = 13))
fat_axis <- ggplot2::theme(axis.line.x = element_line(size = 3, color = axis_line_color), 
                  axis.line.y = element_line(size = 0.5, color = axis_line_color))
h_lines <- ggplot2::theme(panel.grid.major = element_line(size = 0.25, linetype = 3, color = "turquoise4"),
                 panel.grid.major.x = element_blank())
v_lines <- ggplot2::theme(panel.grid.major = element_line(size = 0.25, linetype = 3, color = "turquoise4"),
                 panel.grid.major.y = element_blank())
no_lgnd <- ggplot2::theme(legend.position = "none")
lgnd_bot <- ggplot2::theme(legend.position = "bottom")
lgnd_top <- ggplot2::theme(legend.position = "top")
no_yaxs <- ggplot2::theme(axis.line.y = element_blank(), axis.ticks.y = element_blank(), axis.text.y = element_blank())
...
.param_trace_multi <- function(params = NULL, all_param_names, dat, warmup_val = 0,
  ...
  lgnd_txt <- ggplot2::theme(legend.text = element_text(size = 13, face = "bold"))
  strip_txt <- ggplot2::theme(strip.text = element_text(size = 12, face = "bold", color = "white"),
...
.autocorr_plot <- function(samps, params = NULL, all_param_names,
  ...
  ac_theme <- theme_classic() %+replace% (axis_color + axis_labs + fat_axis + no_lgnd + strip_txt)
  y_scale <- scale_y_continuous(breaks = seq(0, 1, 0.25), labels = c("0","","0.5","",""))
  title_theme <- ggplot2::theme(plot.title = element_text(face = "bold", size = 18))
  if (combine_chains) {
...
.plot_param_vertical_gg <- function(samps,
  ...
  rhat_colors <- scale_color_manual(name = bquote(hat(R)),
                                    values = rhat_pal,
                                    drop = FALSE)
  rhat_lgnd <- ggplot2::theme(legend.position = "top",
  ...
  p.theme <- ggplot2::theme(axis.title=element_blank(),
